...
- Build/refresh the Go binary: `cd kratos-dev/go && make build`, then `./plugins/kratos/bin/kratos init`.
- Old installs: `kratos uninstall` removes legacy global hooks from `~/.claude/settings.json`.
