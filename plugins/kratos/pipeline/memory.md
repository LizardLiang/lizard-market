...
**Default**: Use the Go binary at `<kratos-bin>` for all memory operations. The Python script (`plugins/kratos/memory/kratos_memory.py`) is a legacy fallback only used if the Go binary is unavailable.
...
For highest performance, use the Go binary at `<kratos-bin>`.
