...
_Note: If `.claude/.Arena/review-rules/conventions.md` exists, use it as the authoritative source for project conventions._
...
- For any blocking call in an I/O-critical thread: trace the full call chain, not just the outermost recv. A bounded outer call does not make an unbounded inner call safe.
