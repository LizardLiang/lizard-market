...
- `Bash` only for read-only inspection. The edit gate (`kratos hook edit-gate`) allows these heads: `git status|diff|show|log|branch|rev-parse|ls-files` (with or without `-C <path>`), `ls`, `dir`, `pwd`, `cat`, `type`, `find`, `grep`, `rg`, `sed`, `head`, `tail`, `wc`, `Get-ChildItem|Get-Content|Get-Location`, `Select-String`, `Test-Path` — **plus the read-only kratos subcommands `slug --dated`, `now`, `template get <name>`, `spec validate <slug>`, `spec list`, `agent load|protocol`, `session active`, `pipeline get|status|next|discover`, `memory list`, `feedback list`, `profile list`, `todo list`, `step record-agent|record-file`**.
- The gate classifies **every segment** of the line, not the prefix. So chaining readers is fine (`git status && ls -la`, `grep -rn gate . | head -20`), and a quoted argument is inert (`kratos slug --dated "move the sidebar"`). A single non-reader segment denies the whole line: test runners and package scripts (`go test`, `npm run`), builds, installs, `curl`, a redirect (`> out.txt`), `sed -i`, `find -delete|-exec`, `tail -f`, and a command substitution inside a read (`cat $(…)`). The one accepted decoration is the protocol's timestamp fallback, `TS=$(<kratos-bin> now 2>/dev/null || date -u +%Y-%m-%dT%H:%M:%SZ)`.
