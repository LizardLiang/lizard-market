...
```bash
# 1. Run the CLI
"$KRATOS_BIN" pipeline update --feature <name> --stage 2.5-decomposition --status complete --document decomposition.md
...
"$KRATOS_BIN" step record-agent "$SESSION_ID" daedalus sonnet "<action: e.g. Decomposing <feature> into tasks>"

# Record each document you create
"$KRATOS_BIN" step record-file "$SESSION_ID" ".claude/feature/<name>/decomposition.md" "created"
```
