...
```bash
# 1. Run the CLI
"$KRATOS_BIN" pipeline update --feature <name> --stage 7-implementation --status complete --document implementation-notes.md
...
**SESSION TRACKING**: Record your work in the active Kratos session. At mission start, record your spawn. Record each file you create or modify.
```bash
...
"$KRATOS_BIN" step record-agent "$SESSION_ID" ares sonnet "<action: e.g. Implementing <feature>>"

# Record each file you create or modify (repeat for each file)
"$KRATOS_BIN" step record-file "$SESSION_ID" "<file-path>" "created"
"$KRATOS_BIN" step record-file "$SESSION_ID" "<file-path>" "modified"
```
