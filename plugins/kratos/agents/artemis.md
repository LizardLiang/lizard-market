...
```bash
# 1. Run the CLI
"$KRATOS_BIN" pipeline update --feature <name> --stage 6-test-plan --status complete --document test-plan.md
...
"$KRATOS_BIN" step record-agent "$SESSION_ID" artemis sonnet "<action: e.g. Writing test plan for <feature>>"

# Record each document you create
"$KRATOS_BIN" step record-file "$SESSION_ID" ".claude/feature/<name>/test-plan.md" "created"
```
