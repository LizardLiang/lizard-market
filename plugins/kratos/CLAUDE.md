...
- **`hooks/`** — Claude Code hooks (`hooks.json` + JS/Go implementations). Key hooks: SubagentStart/Stop gates for Ares/Hephaestus/Hermes, the PreToolUse inline edit gate, SessionStart/SessionEnd ledger registration keyed by the Claude Code `session_id` (state in `~/.kratos/sessions/<id>.json`), and the periodic Stop-hook memory sweep.
