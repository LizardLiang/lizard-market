...
| `PreToolUse` | `Write\|Edit\|MultiEdit\|NotebookEdit\|Bash\|PowerShell\|Agent\|Task\|Skill` | `launch.cjs hook edit-gate` | Inline edit gate: denies source edits the inline god should dispatch — Odysseus to plans and spec deltas, Iris to two source files per turn. A `Skill(kratos:<god>)` load records the inline god in the session ledger. A Bash or PowerShell command that reads or uses a stored credential gets `ask` in every context, and a `deny` wins over an `ask`. The gate never allows: a permitted call gets no decision and keeps Claude Code's normal permission prompt, so a classifier miss costs a prompt rather than an unattended `rm -rf` |
| `PreToolUse` | `SubagentHandback` | `launch.cjs hook handback-gate` | Hermes hand-back gate: denies Hermes's one-shot hand-back while a review child it launched has not finished. It reads Hermes's own subagent transcript and keeps no state. After 30 minutes without a launch or a finish it releases the hand-back and names the missing children |
