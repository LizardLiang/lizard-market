#!/usr/bin/env bun
/**
 * Discord channel for Claude Code — discord-remote fork.
 *
 * Forked from the official discord channel plugin. Adds an HTTP sidecar
 * server co-hosted in this process for remote hook approval via Discord DMs.
 *
 * Sidecar additions are marked with "--- discord-remote sidecar ---" comments.
 * The rest of this file is identical to the official plugin's server.ts.
 *
 * Self-contained MCP server with full access control: pairing, allowlists,
 * guild-channel support with mention-triggering. State lives in
 * ~/.claude/channels/discord/access.json — managed by the /discord-remote:access skill.
 *
...
// --- discord-remote sidecar: import sidecar modules ---
...
const client = new Client({
  intents: [
    ...
    // --- discord-remote sidecar: GuildMessageReactions needed for reaction collection ---
...
// The /discord-remote:access skill drops a file at approved/<senderId> when it pairs
...
async function fetchAllowedChannel(id: string) {
  ...
  throw new Error(`channel ${id} is not allowlisted — add via /discord-remote:access`)
}
...
const mcp = new Server(
  { name: 'discord-remote', version: '0.1.0' },
  {
...
const sidecar = createSidecar(client, remoteConfig)

function shutdown(): void {
  if (shuttingDown) return
  shuttingDown = true
  process.stderr.write('discord channel: shutting down\n')
  // --- discord-remote sidecar: clean up sidecar on shutdown ---
...
async function handleInbound(msg: Message): Promise<void> {
  ...
  if (result.action === 'pair') {
    ...
    try {
      await msg.reply(
        `${lead} — run in Claude Code:\n\n/discord-remote:access pair ${result.code}`,
      )
    } catch (err) {
